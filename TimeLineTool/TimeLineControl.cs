...
namespace TimeLineTool
{
    ...
    public enum TimeLineViewLevel { Frame, Frame5, Frame10, Frame20, Frame50, Frame100 };
    ...
    public class TimeLineControl : Canvas
    {
        ...
        private Rectangle _timeIndicator;
        private Rectangle _verticalBar;
        ...
        #endregion
        #region timeIndicatorPosition
        public Double CurrentTimePoint
        {
            get { return (Double)GetValue(CurrentTimePointProperty); }
            set { SetValue(CurrentTimePointProperty, value); }
        }

        // Using a DependencyProperty as the backing store for UnitSize.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty CurrentTimePointProperty =
            DependencyProperty.Register("CurrentTimePoint", typeof(Double), typeof(TimeLineControl),
            new UIPropertyMetadata(1.0,
                new PropertyChangedCallback(OnCurrentTimePointChanged)));
        private static void OnCurrentTimePointChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            TimeLineControl tc = d as TimeLineControl;
            if (tc != null)
            {
                tc.ReDrawTimeIndicator();
        ...
        public static readonly DependencyProperty StartFrameProperty =
            DependencyProperty.Register("StartFrame", typeof(double), typeof(TimeLineControl),
            new UIPropertyMetadata(1.0,
        ...
        public TimeLineControl()
        {
            ...
            _timeIndicator = new Rectangle();
            _timeIndicator.Width = 2;
            _timeIndicator.Height = 80;
            _timeIndicator.Fill = new SolidColorBrush(Colors.OrangeRed);
            _timeIndicator.Cursor = Cursors.SizeWE;
            Canvas.SetLeft(_timeIndicator, 10);
            Canvas.SetTop(_timeIndicator, 0);
            Canvas.SetZIndex(_timeIndicator, 1);
            _timeIndicator.MouseLeftButtonDown += timeIndicator_MouseButtonDown;
            _timeIndicator.MouseLeftButtonUp += timeIndicator_MouseButtonUp;
            _timeIndicator.MouseMove += timeIndicator_MouseMove;
            Children.Add(_gridCanvas);
            Children.Add(_timeIndicator);
            Focusable = true;
            KeyDown += OnKeyDown;
            KeyUp += OnKeyUp;
            MouseEnter += TimeLineControl_MouseEnter;
            MouseLeave += TimeLineControl_MouseLeave;
            PreviewMouseRightButtonDown += timeLineControl_PreviewMouseRightButtonDown;
            //Items = new ObservableCollection<ITimeLineDataItem>();
            SizeChanged += TimeLineSizeChanged;
        ...
        void Items_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == System.Collections.Specialized.NotifyCollectionChangedAction.Add)
            {
                ...
                if (e.NewStartingIndex == Items.Count())//this is add action, we need to do differenthing because this cause by both drop and cut action
                {
                    if(_justDropped)
                    {
                        _justDropped = false;
                    ...
                    {
                        double duration = itm.EndFrame - itm.StartFrame;
                        if (Items.Count == 1)//this is the first one added
                        {
                            itm.StartFrame = 0;
                            itm.EndFrame = duration;
                        }
                        else
                        {
                            var last = Items.OrderBy(i => i.StartFrame).LastOrDefault();
                            if (last != null)
                            {
                                itm.StartFrame = last.EndFrame;
                                itm.EndFrame = itm.StartFrame + duration;
                            }
                        }
                        ...
                        Children.Insert(Items.Count + 1, ctrl);
                    }
                   
                }
                else // this is insert action
                {
                    if (_justDropped)
                    {
                        _justDropped = false;
                    ...
                    {
                        var ctrl = CreateTimeLineItemControl(itm);
                        Children.Insert(e.NewStartingIndex + 2, ctrl);
                    }
                }
            }
        ...
        private TimeLineItemControl CreateTimeLineItemControl(ITimeLineDataItem data)
        {
            ...
            Binding trimStartBinding = new Binding("TrimStart");
            trimStartBinding.Mode = BindingMode.TwoWay;
            trimStartBinding.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            Binding trimEndBinding = new Binding("TrimEnd");
            trimEndBinding.Mode = BindingMode.TwoWay;
            trimEndBinding.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            Binding originalDuration = new Binding("OriginalDuration");
            originalDuration.Mode = BindingMode.TwoWay;
            originalDuration.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
            double timelineStart = StartFrame;
            ...
            adder.TimeLineStartFrame = timelineStart;
            adder.DataContext = data;
            adder.Content = data;

            adder.SetBinding(TimeLineItemControl.StartFrameProperty, startBinding);
            adder.SetBinding(TimeLineItemControl.EndFrameProperty, endBinding);
            adder.SetBinding(TimeLineItemControl.IsExpandedProperty, expandedBinding);
            adder.SetBinding(TimeLineItemControl.TrimStartProperty, trimStartBinding);
            adder.SetBinding(TimeLineItemControl.TrimEndProperty, trimEndBinding);
            adder.SetBinding(TimeLineItemControl.OriginalDurationProperty, originalDuration);
        ...
        internal void ReDrawTimeIndicator()
        {
            var newLeft = ConvertFrameToDistance((Double)GetValue(CurrentTimePointProperty));
            if (newLeft < 0)
                newLeft = 0;
            Canvas.SetLeft(_timeIndicator, newLeft);
        }
        ...
        private void ReDrawChildren()
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                if (mover != null)
                {
                    mover.TimeLineStartFrame = start;
        ...
        #endregion
        private void TimeLineSizeChanged(object sender, System.Windows.SizeChangedEventArgs e)
        {
            if (e.PreviousSize.Height != Height)
            {
                DrawBackGround();
            }
        }
        ...
        private void DrawBackGround(Boolean isDrawGridUpdate = false)
        {
                ...
                Items == null)
            {
                ...
                bg.Stroke = Application.Current.Resources["DarkMaskBrush"] as Brush;
                Width = bg.Width;
                Height = bg.Height;
        ...
        private void DrawTimeGridExecute()
        {
            ...
            //place our gridlines
            //DrawFrame120Lines(grid);
            //DrawFrame60Lines(grid);
            //DrawFrameLines(grid);
        }
        //private void DrawFrameLines(Canvas grid) // draw frame line
        //{
        //    Double frame30Size = UnitSize / 2;
        //    Double frame15Size = UnitSize / 4;
        //    Double frameSize = UnitSize / 60;
        ...
        //}
        //private void DrawFrame60Lines(Canvas grid)
        //{
        //    Double hourSize = UnitSize;
        ...
        //    }
        //}
        //private void DrawFrame120Lines(Canvas grid)
        //{
        ...
        void timeLineControl_PreviewMouseRightButtonDown(object sender, MouseButtonEventArgs e)
        {
            //var tc = sender as TimeLineControl;
            ////set curent frame at this position
            //var currentPosition = Mouse.GetPosition(tc).X;
            //Canvas.SetLeft(_timeIndicator, currentPosition);
            ////set time point respectively
            //CurrentTimePoint = ConvertDistanceToFrame(currentPosition);
        }
            ...
            {//GongSolutions.Wpf.DragDrop
                ...
                if (d2 != null)
                {
                    ...
                    if (DragAdorner == null)
                    {
                        ...
                        _tmpDraggAdornerControl.StartFrame = StartFrame;
        ...
        void TimeLineControl_Drop(object sender, DragEventArgs e)
        {
            ...
            ITimeLineDataItem cloneDropData = null;
            if (dropper == null)
            {
                ...
                if (dropData != null)
                {
                    ...
                    //clone this data
                    cloneDropData = new TempDataType();
                    cloneDropData.Color = dropData.Color;
                    cloneDropData.Source = dropData.Source;
                    cloneDropData.Name = dropData.Name;
                    cloneDropData.StartFrame = 0.0;
                    cloneDropData.EndFrame = dropData.EndFrame;
                    cloneDropData.OriginalDuration = dropData.OriginalDuration;
                    cloneDropData.TimelineViewExpanded = false;

                    dropper = CreateTimeLineItemControl(cloneDropData);
                    dropper.StartFrame = StartFrame;
        ...
        #region drop helpers
        private bool _justDropped; // this is to know the new item occured by drop or by viewmodel
        ...
        {
            //block context menu after right click and moving

            (sender as TimeLineItemControl).ContextMenuOpening += _contextMenu_Handler;
            Cursor = Cursors.Arrow;
            (sender as TimeLineItemControl).ReleaseMouseCapture();
            Keyboard.Focus(this);
        }
        private void _contextMenu_Handler(object sender, RoutedEventArgs e)
        {
            if (_justMoved)
            {
                e.Handled = true;
        ...
        internal void HandleItemManipulation(TimeLineItemControl ctrl, TimeLineItemChangedEventArgs e)
        {
            ...
            int afterIndex = -1;
            int previousIndex = -1;
            after = GetTimeLineItemControlStartingAfter(ctrl.StartFrame, ref afterIndex);
            previous = GetTimeLineItemControlStartingBefore(ctrl.StartFrame, ref previousIndex);
            if (after != null)
        ...
        void item_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            Double deltaT = ctrl.GetDeltaFrame(deltaX);
        ...
        void timeIndicator_MouseButtonDown(object sender, MouseButtonEventArgs e)
        {
            Rectangle rct = sender as Rectangle;
            if (rct != null)
            {
                rct.CaptureMouse();
        ...
        void timeIndicator_MouseButtonUp(object sender, MouseButtonEventArgs e)
        {
            Rectangle rct = sender as Rectangle;
            if (rct != null)
            {
        ...
        void timeIndicator_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            #region drag - left click and move
            Rectangle ctrl = sender as Rectangle;
            if (ctrl == null)
                return;
            Double mouseX = Mouse.GetPosition(null).X;
            Double deltaX = mouseX - _curX;
            Double deltaF = ConvertFrameToDistance(deltaX);
            var newLeft = Canvas.GetLeft(_timeIndicator) + deltaX;
            if (Mouse.LeftButton == MouseButtonState.Pressed)
            {
                if (Mouse.Captured != ctrl)
                {
                ...
                if (newLeft < 0)
                    newLeft = 0;
                Canvas.SetLeft(_timeIndicator, newLeft);
                CurrentTimePoint = ConvertDistanceToFrame(newLeft);
        ...
        private TimeLineItemControl GetTimeLineItemControlStartingBefore(double frame, ref int index)
        {
            index = -1;
            for (int i = 0; i < Items.Count; i++)
            {
                var checker = GetTimeLineItemControlAt(i);
                if (checker != null && checker.StartFrame == frame && i != 0)
                {
                    index = i - 1;
        ...
        private TimeLineItemControl GetTimeLineItemControlStartingAfter(double frame, ref int index)
        {
            for (int i = 0; i < Items.Count; i++)
            {
                var checker = GetTimeLineItemControlAt(i);
                if (checker != null && checker.StartFrame > frame)
                {
                    index = i;
                    return checker;
                }
