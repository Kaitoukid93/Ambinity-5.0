...
namespace adrilight
{
    internal class CompositionPlayer : ICompositionPlayer
    {


        private readonly NLog.ILogger _log = LogManager.GetCurrentClassLogger();

        public CompositionPlayer(IOutputSettings outputSettings, IRainbowTicker rainbowTicker, IGeneralSettings generalSettings, MainViewViewModel mainViewViewModel)
        {
        ...
        private void RefreshColorState()
        {
            ...
            var shouldBeRunning = OutputSettings.OutputIsEnabled && OutputSettings.OutputParrentIsEnable && OutputSettings.OutputSelectedMode == 5 && OutputSettings.IsInSpotEditWizard == false;
...

        {
            ...
            try
            {
                ...
                // get all involve parameters that does not change during playback
                // get current compositon ( called Selected Composition)
                // scale down Selected Composition to the exact number of LEDs in the output
                // the output of this step called NativeComposition
                var selectedComposition = new Composition() // testing purpose
                    {
                    Name = "DoubleBouncing",
                    Owner = "ambino",
                    Type = "Moving Object",
                    Description = "Created by Ae",
                ...
                {
                    //read frame one by one and show  to the LEDs
                    // the frames is scale down to the exact number of LEDs(spot) at the above steps
